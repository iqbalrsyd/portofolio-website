	...
	{:else}
		<FancyBanner img={banner}>
			<div class="flex w-full flex-col items-center justify-center gap-4">
				...
				{#if data.item.category}
					<Badge variant="secondary" class="text-base">{data.item.category.name}</Badge>
				{/if}
			</div>
		</FancyBanner>
		<Separator />
		...
		{#if related.length !== 0}
			<div class="flex flex-col gap-4 px-4 py-6">
				<H3>Related Projects & Experience</H3>
				<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
					{#each related as item}
						<a href={href(item.link)} class="transition-transform hover:scale-105">
							<Card class="h-full">
								<CardHeader class="flex flex-row items-center gap-4">
									<img src={item.logo} alt={item.name} class="h-12 w-12 rounded-md object-contain" />
